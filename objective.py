...
class Objective(BaseObjective):
    ...
    def skip(self, **data):
        if self.procedure == "train_test_split" and self.n_splits is not None:
            return True, "train_test_split does not require n_splits"
        if self.procedure == "KFold" and \
                self.n_splits != int(1.0 / self.test_size):
            return True, (
                "The KFold's n_splits parameter must be equal to 1/test_size"
                )
